...
fn inlier_parquet(path: &str) {
    ...
    for row in iter {
        ...
        let pcxyzi_in_start = Instant::now();
        let inliers_ref = ransac(&dsxyzi, 10, 0.5);
        let inxyzi = PointCloudXYZIntensity::from(inliers_ref);
        let pcxyzi_in_loaded = Instant::now();
        println!(
            "{:?} ransac {:?}",
            pcxyzi_in_loaded.duration_since(pcxyzi_in_start),
            inxyzi.num_points()
        );
    }
}
...
pub fn main() {
    inlier_parquet("/u01/data/poc4/20200718/point_cloud2_20200718144724.parquet");
}
